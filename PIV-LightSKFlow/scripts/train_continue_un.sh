...
# PIV单阶段训练脚本
TIME=$(date "+%Y%m%d_%H%M%S")
MODEL=SK_Decoder  # 保持与模型代码一致

PYTHON=python3
PIV_ROOT=/home/lin/PIV-SKFlow-main/PIV_datasets # 修改为实际路径

# 输出目录配置
SAVE_DIR=results/piv/${MODEL}-${TIME}

# PIV训练阶段
CUDA_VISIBLE_DEVICES=0,1 ${PYTHON} train_continue_modified.py \
--name piv_experiment \
--stage single \
--output ${SAVE_DIR} \
--restore_ckpt piv_experiment_step_1500000.pth \
--num_steps 100000 \
--lr 0.00001 \
--image_size 512 512 \
--wdecay 0.00005 \
